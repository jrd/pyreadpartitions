#!/usr/bin/env python3
# vim:et:sta:sts=4:sw=4:ts=4:tw=0:
"""
Read MBR (msdos) and GPT partitions for a disk on UNIX.
Does not rely on any os tools nor library.
...
GPT_GUID = {
    ...
    'BFBFAFE7-A34F-448A-9A5B-6213EB736C22': 'Lenovo boot partition',  # or 'Ceph Journal'
...
def read_mbr_header(fp):
    ...
    data = fp.read(calcsize(fmt))
    header = MBRHeader._make(unpack(fmt, data))
...
def read_mbr_partitions(fp, header):
    def read_mbr_partition(partstr, num):
        ...
        part = MBRPartition._make(unpack(fmt, partstr) + (num, False, ''))
        if part.type:
            ptype = 'Unknown'
...
def read_gpt_header(fp, lba_size=512):
    ...
    data = fp.read(calcsize(fmt))
    header = GPTHeader._make(unpack(fmt, data))
    if header.signature != b'EFI PART':
        raise GPTMissing('Bad GPT signature')
    revision = header.revision_major + (header.revision_minor / 10)
    if revision < 1.0:
        raise GPTError(f'Bad GPT revision: {header.revision_major}.{header.revision_minor}')
    if header.header_size < 92:
        raise GPTError(f'Bad GPT header size: {header.header_size}')
    header = header._replace(
        disk_guid=str(UUID(bytes_le=header.disk_guid)).upper(),
    )
    return header
...
def read_gpt_partitions(fp, header, lba_size=512):
    ...
    for i in range(header.num_part_entries):
        ...
        if len(data) < calcsize(fmt):
            raise GPTError(f'Short GPT partition entry #{i + 1}')
        part = GPTPartition._make(unpack(fmt, data) + (i + 1, ''))
        if part.guid == 16 * b'\x00':
            continue
        guid = str(UUID(bytes_le=part.guid)).upper()
        ptype = 'Unknown'
        if guid in GPT_GUID:
            ptype = GPT_GUID[guid]
        part = part._replace(
            guid=guid,
            uid=str(UUID(bytes_le=part.uid)).upper(),
...
def check_disk_file(disk):
    try:
        disk.tell()
        if hasattr(disk, 'encoding'):
            raise OSError("file is opened as text")
    except OSError:
        raise DiskException('Please provide a file pointer opened in binary as first argument, pointing to an existing disk such as /dev/sda.')
...
def get_gpt_info(disk):
    ...
    try:
        blocksize = unpack('i', ioctl(disk.fileno(), 4608 | 104, pack('i', -1)))[0]
    except OSError:
        blocksize = 512
    try:
...
def show_disk_partitions_info(diskOrInfo):
    ...
    if info.mbr:
        mbr = info.mbr
        print('MBR Header')
        print(f'LBA size (sector size): {mbr.lba_size}')
        print(f'Number of MBR partitions: {len(mbr.partitions)}')
        print('legend: # -> position, #s -> in number of sectors\n')
        print('#  Active From(#s)   Size(#s)   Code Type')
        for part in mbr.partitions:
            print(f"{part.index:<2} {('*' if part.active else '_'):^6} {part.lba:>10} {part.sectors:>10} {part.type:^4X} {part.type_str}")
        print('')
    else:
        print('No MBR')
    print('---')
    if info.gpt:
        gpt = info.gpt
        print('GPT Header')
        print(f'Disk GUID: {gpt.disk_guid}')
        print(f'LBA size (sector size): {gpt.lba_size}')
        print(f'GPT First LBA: {gpt.current_lba}')
        print(f'GPT Last  LBA: {gpt.backup_lba}')
        print(f'Number of GPT partitions: {len(gpt.partitions)}')
        print('legend: # -> position, #s -> in number of sectors\n')
        print('#   Flags From(#s)   To/Size(#s)  GUID/UID                             Type/Name')
        for part in gpt.partitions:
            print(f"{part.index:<3} {(part.flags if part.flags else '_'):^5} {part.first_lba:>10} {part.last_lba:>12} {part.guid} {part.type}")
            print(' ' * (3 + 1 + 5 + 1 + 10 + 1) + f'{(part.last_lba - part.first_lba + 1):>12} {part.uid} {part.name}')
        print('')
    else:
        print('No GPT')
    print('---')
...
def main():
    if len(argv) > 1:
        ...
        d_stdin = stdin.detach()
        fp = BytesIO()
        fp.write(d_stdin.read())
        fp.seek(0)
    try:
        show_disk_partitions_info(fp)
        return 0
